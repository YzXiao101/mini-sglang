...
def iter_filtered_prompt_ids(tokenizer):
    ...
    for batch in parquet.iter_batches(batch_size=256, columns=["conversation"]):
        ...
        all_ids = tokenizer(prompts, add_special_tokens=False)["input_ids"]
        for ids in all_ids:
            yield ids
...
def main() -> None:
    ...
    print(
        "Warmup sample: "
        f"input={len(prompt_token_ids[0])}tok, output={len(warmup_result["token_ids"])}tok, "
        f"preview='{warmup_result["text"]}'"
    )
